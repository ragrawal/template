...
description: Conventions for writing and extending this repo's pytest-bdd (Given/When/Then) test suite under tests/bdd/.
user-invocable: true
---
...
tests/
  ...
  bdd/
    ...
    python_project_test/
      __init__.py
      python_project_test.feature   # Gherkin scenarios
      test_python_project_test.py   # binds the feature file, nothing else
```
...
- Scenario text must be literal and runnable: real CLI commands (e.g.
  `uv tool run copier copy ...`, `uv run poe check`), real file paths, real
  substrings to check for — not paraphrased business language. A reader
  should be able to copy a `When`/`Then` line and run it by hand.
...
- The binding file, e.g. `tests/bdd/python_project_test/test_python_project_test.py`,
  should normally contain nothing but:
  ```python
...
The fix used throughout this repo: import each action module with a
...
uv tool run ruff check --select F401,F841 tests/  # unused imports/vars after moving code around
```
